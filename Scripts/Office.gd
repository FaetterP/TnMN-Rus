...
const MIN_NUMBER_DOPPEL_CHAR = 3 #3
const MAX_NUMBER_DOPPEL_CHAR = 5 #5
...
func _neighbor_in() -> void:
	if chars_level_list.is_empty():
		...
		if Global.unlikely and new_char == "Clown":
...
func _spawn_character(character) -> void:
	...
	if is_not_on_today_list:
		...
		if Global.language == SPANISH_VALUE:
			...
			if random_excuse == 0:
				if characters.get_child(0).name_char == "Mclooy_Rudboys":
					characters.get_child(0).question_answers_spanish["today_list_ok"] = ["我今天不在名单上，因为我不得不因为紧急情况离开。"]
				else:
					characters.get_child(0).question_answers_english["today_list_ok"] = ["我今天不在名单上，因为我因工作紧急情况不得不离开。"]
			else:
				characters.get_child(0).question_answers_english["today_list_ok"] = ["那么，一切都正常吗？","你能为我开门吗？"]
	if is_3312_active:
